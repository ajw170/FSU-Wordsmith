/*
    ...
    general test harness for alt2c::List
...
typedef alt2c::List      < ElementType > ContainerType;
...
int main( int argc, char* argv[] )
{
  ...
  if (argc > 1)
  {
    if (argv[1][0] == '-')
    {
      ...
      {
        case 'h': case 'H':
          std::cout << " command line arguements (optional):\n"
                    << "   1: batch command file name; commands duplexed to screen in color (optional)\n"
                    << "   2: output file name; commands duplexed to file; color suppressed (optional)\n";
          exit (EXIT_SUCCESS);
          break;
              
        default:
          std::cout << " ** unkown flag \'" << argv[1][1] << "\' detected\n"
                    << "    try again\n";
          exit (EXIT_SUCCESS);
      }
    }
  ...
  do
  {
    ...
    *isptr >> option;
    ...
    switch (option)
    {
	  ...
	  case 'd': case 'D': // Display and Dump
            if (BATCH) *osptr << '\n';
	  ...
	  case 't':  case 'T': // traverse
          ...
          case 's':  case 'S': // unsigned int Size()
            ...
            *osptr << "  " << x1.Excess() << " = x1.Excess();\n";
            *osptr << "  " << x1.Capacity() << " = x1.Capacity();\n";
	    break;
	  ...
	  case 't':  case 'T': // traverse
          ...
          case 's':  case 'S': // unsigned int Size()
            ...
            *osptr << "  " << x2.Excess() << " = x2.Excess();\n";
            *osptr << "  " << x2.Capacity() << " = x2.Capacity();\n";
	    break;
	  ...
	  case 't':  case 'T': // traverse
          ...
          case 's':  case 'S': // unsigned int Size()
            ...
            *osptr << "  " << x3.Excess() << " = x3.Excess();\n";
            *osptr << "  " << x3.Capacity() << " = x3.Capacity();\n";
	    break;
...
void Preamble(std::ostream& os)
{
  os
     ...
     << "      1>        x1.Sort(pred)\n"
     << "      1m2       x1.Merge(x2)\n"
     << "      1M2       x1.Merge(x2,pred)\n"
     << "      3=2       x3 = x2\n"
     << "      =         x3 = x2 = x1; init i1, i2, i3; CopyTest(x3); AssignTest(x3);\n"
     << "   Note that 'pred' is a GreaterThan<T> object\n"
     << std::endl;
}
